...
const Navbar = ({ scrollToSection, refs, activeSection }) => {
  ...
  return (
    <nav className={`text-white flex justify-around p-3 h-20 cursor-pointer items-center ${isFixed ? 'fixed top-0 w-full bg-black bg-opacity-90 shadow-lg' : 'relative bg-transparent'}`}>
      <div>
        <h3 className='md:text-xl text-sm lg:text-2xl'>ACHAL SAWANT</h3>
      </div>
      <div className='flex md:gap-16 items-center gap-6'>
        <ul className={`flex flex-col md:flex-row md:gap-6 lg:gap-16 gap-4 items-center ${isOpen ? 'flex' : 'hidden md:flex'} absolute md:static top-20 left-0 w-full bg-black bg-opacity-80 md:bg-transparent text-center md:text-left p-8 md:p-0 z-50`}>
